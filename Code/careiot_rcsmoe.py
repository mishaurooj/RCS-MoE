...
ABLATIONS = {
    "full_rcsmoe": {},
    "no_rarity_conditioning": {"use_rarity": False},
    "no_sparse_experts": {"use_moe": False},
    "no_cost_objective": {"use_cost_loss": False},
    "no_family_objective": {"use_family_loss": False},
    "no_ood_reconstruction": {"use_reconstruction": False},
    "fixed_edge_only": {"force_route": "edge"},
    "fixed_cloud_only": {"force_route": "cloud"},
}
...
class JointObjective(nn.Module):
    ...
    def forward(
        ...
        # Route targets are available only during training.
        # rare -> cloud, uncertain common -> specialist, easy common -> edge
        with torch.no_grad():
            edge_conf = torch.softmax(
                output["edge_logits"], dim=-1
            ).max(dim=-1).values
            route_target = torch.zeros_like(y)
            route_target[(edge_conf < 0.80) & (rare < 0.5)] = 1
            route_target[rare >= 0.5] = 2
